...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("Eduardo Rampon Meireles - Cubo"); // Criar Janela
...
void conf_luz(void)
{
	...
	glEnable(GL_LIGHTING); // suporte a iluminação
	glEnable(GL_LIGHT0);
}
...
void keyboard(unsigned char key, int a, int b)
{
    switch (key) {
        case 'o':
            ...
            break;
        case 'c':
            float ambCobre[] = { 0.0, 0.0, 0.0, 1.0 };
            float ambInox[] = { 0.24f, 0.24f, 0.24f, 1.0f };

            float diffCobre[] = { 0.7, 0.35, 0.2, 1.0 };
            float diffInox[] = { 0.28f, 0.28f, 0.28f, 1.0f };

            float specCobre[] = { 1.0, 0.0, 0.0, 1.0 };
            float specInox[] = { 0.78f, 0.78f, 0.78f, 1.0f };

            float brightnessCobre[] = { 28.0f };
            float brightnessInox[] = { 28.0f };

            if (on) {
                for (int i = 0; i < 3; i++) {
                    mat_diffuse[i] = diffCobre[i];
                    mat_ambiente[i] = ambCobre[i];
                    mat_specular[i] = specCobre[i];
                }

                brilho[0] = brightnessCobre[0];

                on = 0;
            }
            else {
                for (int i = 0; i < 3; i++) {
                    mat_diffuse[i] = diffInox[i];
                    mat_ambiente[i] = ambInox[i];
                    mat_specular[i] = specInox[i];
                }

                brilho[0] = brightnessInox[0];
                on = 1;
            }
		    glutPostRedisplay();
		    break;
